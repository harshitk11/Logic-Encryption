...
int     main( int argc, char *argv[] )
{
	...
	NO_HOPE_KEYNODE = read_hope_keynode();			// For reading the contents of the file hope_key_node.txt and storing it in the array hope_key_node[]
...
for(int z = 0; z < 200; z++)			// Iterating over all the hope_key_node to find the dependency of each node in the list of hope_key_node 
{	
		...
		strcat(output_dependency, hope_key_node[z] );
	...
	{	
		...
		strcpy(output_dependency, circuit_name);		// This creates a new file (of the name of output) inside <circuit_name>/dependency/<hope_key_node[z]>/
		strcat(output_dependency, "/dependency/");
		strcat(output_dependency, hope_key_node[z] );
		...
		*/
		...
		while(path_counter>0)
		{
			...
			{	
				...
				if(!(strcmp(temp,node_array[jj].name)) && strcmp(temp,hope_key_node[z]))
				{
...
int read_hope_keynode()
{
	...
	NO_HOPE_KEYNODE = j;
	for(int i = 0; i<NO_HOPE_KEYNODE; i++)			// Printing the nodes in the file hope_key_node.txt. Change the value of index as per your need.
		{	
			printf("%s\n",hope_key_node[i]);
		}
	
	return NO_HOPE_KEYNODE;
