...
void find_path()
{
	...
	FILE *path_history, *dff_history, *not_history, *or_history, *nor_history, *and_history, *nand_history;
	char input_list[100];
	char affected_dff_list[100];

	char affected_not_list[100];
	char affected_or_list[100];
	char affected_nor_list[100];
	char affected_and_list[100];
	char affected_nand_list[100];	

	strcpy(input_list, circuit_name);
	strcat(input_list, "/");
	strcpy(affected_dff_list, input_list);

	strcpy(affected_not_list, input_list);
	strcpy(affected_or_list, input_list);
	strcpy(affected_nor_list, input_list);
	strcpy(affected_and_list, input_list);
	strcpy(affected_nand_list, input_list);

	strcat(input_list, "inputs_affecting_each_output.txt");
	strcat(affected_dff_list, "dffs_affecting_each_output.txt");
	
	strcat(affected_not_list, "not_affecting_each_output.txt");
	strcat(affected_or_list, "or_affecting_each_output.txt");
	strcat(affected_nor_list, "nor_affecting_each_output.txt");
	strcat(affected_and_list, "and_affecting_each_output.txt");
	strcat(affected_nand_list, "nand_affecting_each_output.txt");
	
	path_history = fopen(input_list,"w");
	dff_history = fopen(affected_dff_list,"w");
	not_history = fopen(affected_not_list,"w");
	or_history = fopen(affected_or_list,"w");
	nor_history = fopen(affected_nor_list,"w");
	and_history = fopen(affected_and_list,"w");
	nand_history = fopen(affected_nand_list,"w");
      ...
      node_array[i].color=0;
		...
		for(int jj = 0; jj< count; jj++)
		{
			if(!(strcmp(output_name[ii],node_array[jj].name)))
			{
				...
				fprintf(not_history,"%s\t====>", node_array[jj].name);
				fprintf(or_history,"%s\t====>", node_array[jj].name);
				fprintf(nor_history,"%s\t====>", node_array[jj].name);
				fprintf(and_history,"%s\t====>", node_array[jj].name);
				fprintf(nand_history,"%s\t====>", node_array[jj].name);
		...
		while(path_counter>0)
		{
			...
			for(int jj = 0; jj < count; jj++)
			{
				if(!(strcmp(temp,node_array[jj].name)))
				{
					...
					else if(!(strcmp(node_array[jj].type, "NOT")))
					{
						fprintf(not_history,"%s\t", node_array[jj].name);					

					}
					else if(!(strcmp(node_array[jj].type, "OR")))
					{
						fprintf(or_history,"%s\t", node_array[jj].name);
					}
					else if(!(strcmp(node_array[jj].type, "NOR")))
					{
						fprintf(nor_history,"%s\t", node_array[jj].name);
					}
					else if(!(strcmp(node_array[jj].type, "AND")))
					{
						fprintf(and_history,"%s\t", node_array[jj].name);	
					}
					else if(!(strcmp(node_array[jj].type, "NAND")))
					{
						fprintf(nand_history,"%s\t", node_array[jj].name);
					}
