...
int main()
{
	...
	while(current_key_no <= key_size )
	{
		...
		system("./hope -r 1000 -1 -N -D s38584.1.bench -l s38584.1.txt");
		FILE *read_hope_output, *write_zero, *write_one, *write_fault, *write_fault_list, *read_bench, *write_bench, *read_bench1, *read_bench2;
	
		read_hope_output = fopen ("s38584.1.txt", "r");
		//write_zero = fopen("s38584.1_zero_stat.txt","w");
		//write_one = fopen("s38584.1_one_stat.txt","w");
		//write_fault = fopen("s38584.1_fault_stat.txt","w");
		//write_fault_list = fopen("s38584.1_fault_list.txt","w");
		...
		read_bench = fopen("s38584.1.bench","r");
		write_bench = fopen ("s38584.1_mod.bench","w");
		read_bench1 = fopen("s38584.1.bench","r");
		read_bench2 = fopen("s38584.1.bench","r");
		...
		read_bench = fopen("s38584.1_mod.bench","r");
		write_bench = fopen("s38584.1.bench","w");
		while(!feof(read_bench))
		{
			fscanf(read_bench, "%[^\n]\n", temp);
			fprintf(write_bench,"%s\n", temp);
