...
typedef uint8_t state_t[4][4];	   // state_t is the datatype for uint8_t[4][4]		
static state_t* state;						

FILE *fp;	// fp stores the encrypt_states and the decrypt_states
...
void printBits(unsigned n)			// Takes as input an integer and prints binary equivalent into a file
{
    unsigned i;
    for (i = 1 << 7; i > 0; i = i / 2)
...
static void AddRoundKey(uint8_t round)
{
  ...
  for (i=0;i<4;++i)
  {
    for (j = 0; j < 4; ++j)
    {
      ...
      //printBits(sizeof(uint8_t), (*state)[i][j]);
      printBits((*state)[i][j]);
    }
    fprintf(fp,"\n");
  }
